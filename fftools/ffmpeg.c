...
/*
 * Send a single packet to the output, applying any bitstream filters associated with the output stream.
 * This may result in any number of packets actually being written, depending on what bitstream filters are applied.
 * The supplied packet is consumed and will be blank (as if newly-allocated) when this function returns.
 * If eof is set, instead indicate EOF to all bitstream filters and therefore flush any delayed packets to the output.
 * A blank packet must be supplied in this case.
 */
...
static void do_video_out(OutputFile *of, OutputStream *ost, AVFrame *next_picture, double sync_ipts)
{
    ...
    for (i = 0; i < nb_frames; i++)
    {
        ...
        if (!check_recording_time(ost)) return; // ✖️ not reached
...
void do_test()
{
    //av_log(NULL, AV_LOG_ERROR, ">>> exit_on_error %d, nb_output_streams %d\n", exit_on_error, nb_output_streams); // 0do_video_out, 2
...
static int reap_filters(int flush)
{
    ...
    /* Reap all buffers present in the buffer sinks(洗涤池) */
    for (i = 0; i < nb_output_streams; i++) {
        OutputStream *ost = output_streams[i];
        OutputFile    *of = output_files[ost->file_index];
        ...
        while (1) {
...
#if 0
            if (enc->codec_type == AVMEDIA_TYPE_VIDEO)
                av_log(NULL, AV_LOG_ERROR, "%d:%s %dx%d, rc %d, wc %d, ret %d, pts %lld\n", enc->codec_type, enc->codec_type ? "Audio" : "Video",
                       filtered_frame->width, filtered_frame->height, rc, ++wc, ret, filtered_frame->pts);
            else
                av_log(NULL, AV_LOG_ERROR, "%d:%s %dx%d, rc %d, wc %d, ret %d\n", enc->codec_type, enc->codec_type ? "Audio" : "Video",
                       filtered_frame->width, filtered_frame->height, rc, ++wc, ret);
#endif
            ...
            if (ret < 0) {
                if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                    ...
                    if (av_buffersink_get_type(filter) == AVMEDIA_TYPE_VIDEO && !last_written)
                    {
            ...
            if (filtered_frame->pts != AV_NOPTS_VALUE) {
                ...
                //float_pts += FFSIGN(float_pts) * 1.0 / (1<<17); // pts 多一个约 0.0000076 的尾巴。注释之后，没发现有实际的影响
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                do_test();
                do_video_out(of, ost, filtered_frame, float_pts); // TODO: ret -1 exit_program(1)
                break;
...
static int check_init_output_file(OutputFile *of, int file_index)
{
    ...
    for (i = 0; i < of->ctx->nb_streams; i++) {
        ...
        while (av_fifo_size(ost->muxing_queue)) {
            AVPacket pkt;
            av_fifo_generic_read(ost->muxing_queue, &pkt, sizeof(pkt), NULL);
            main_return_code = write_packet(of, &pkt, ost, 1);
        }
    }
...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_filtergraphs; i++) { // nb_filtergraphs: 2 [audio, video]
        FilterGraph *fg = filtergraphs[i];
        for (j = 0; j < fg->nb_outputs; j++) {
