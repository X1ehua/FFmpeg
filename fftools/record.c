...
// int write_packet(AVFormatContext *s, AVPacket *pkt, OutputStream *ost, int unqueue)
{
    ...
    if (!(s->oformat->flags & AVFMT_NOTIMESTAMPS)) {
            ...
            pkt->dts != AV_NOPTS_VALUE && !(st->codecpar->codec_id == AV_CODEC_ID_VP9 && ost->stream_copy) && ost->last_mux_dts != AV_NOPTS_VALUE)
        {
            ...
            if (pkt->dts < max) {
                ...
                av_log(s, loglevel, "Non-monotonous DTS in output stream %d:%d; previous: %lld, current: %lld; ",
                ...
                */
                ...
                av_log(s, loglevel, "changing to %lld. This may result in incorrect timestamps in the output file.\n", max);
                if (pkt->pts >= pkt->dts)
